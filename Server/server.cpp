...
{
  using namespace std;
  int sockfd, portno;  // These two variables store the values returned by the
  ...
  listen(sockfd,
         10);  // process to listen for socket for cons, 5 pending max in queue
  ...
  while (1) {  // main accept loop
    ...
    connectionFD = accept(sockfd, (struct sockaddr *)NULL,
                          NULL);  // blocks until a client connects
    if (connectionFD >= 0) {      // No error codes{
      Session **ptr =
          (Session **)malloc(sizeof(Session *) * (numConnections + 1));
      ...
      if (!ptr || !newSession) {
        ...
        for (int i = 0; i < numManFiles; i++) {
          ...
          // len = snprintf(writeBuffer,1024,"%s %i %s
          // ",filePointer[i].fileName,filePointer[i].size,filePointer[i].hash);
          strncpy(msg.fileName, filePointer[i].fileName,
                  sizeof(filePointer[i].fileName));
      ...
      for (int x = 0; x < numConnections; ++x) {
        const int len = recv(session[x]->get_sockfd(), (void *)buffer,
        ...
        if (len > 0) {
          ...
          while (remaining_len > 0) {
            if (buffer[0] == SERVER_FILE_REQ) {
              ...
              if (remaining_len >= 257) {
                ...
                bool found = false;
                for (int i = 0; i < numManFiles; i++) {
                  if (strncmp(filePointer[i].fileName, req.fileName, 256) ==
                      0) {
                    ...
                    snprintf(formattedName, 284, "%s%s", prepend,
                             filePointer[i].fileName);
                    ...
                    memcpy(send.fileName, filePointer[i].fileName,
                           sizeof(filePointer[i].fileName));
                    ...
                    printf("Doing Malloc for MSG Here. FileSize:%u\n",
                           filePointer[i].size);
                    char *newMsg =
                        (char *)malloc(sizeof(send) + filePointer[i].size);
                    ...
                    printf("\n");
                    memcpy(&newMsg[sizeof(send)], fileContents,
                           filePointer[i].size);
                    bWritten = write(session[x]->get_sockfd(), newMsg,
                                     sizeof(send) + filePointer[i].size);
                    ...
                    printf("\n");
                    if (bWritten ==
                        (sizeof(send) + filePointer[i].size)) {  // good
          ...
          delete session[x];
          numConnections--;
          session[x] = session[numConnections];  // last connection replaces
                                                 // deleted connection
...
inline int Session::get_sockfd() { return sockfd; }

uint32_t Session::get_user_id() { return user_id; }
...
struct fileInfo *checkDir() {
  ...
  if (d) {
    ...
    printf("Number of files in directory: %i\n", numManFiles);
    filePointer2 =
        (struct fileInfo *)malloc(sizeof(struct fileInfo) * numManFiles);
    rewinddir(d);
    int dirIndex = 0;
    ...
    while ((dir = readdir(d)) != NULL) {
      if (strstr(dir->d_name, search) != NULL) {
        ...
        printHash(hash);
        fclose(fp);
        strncpy(filePointer2[dirIndex].fileName, dir->d_name,
                sizeof(filePointer2[dirIndex].fileName));
        filePointer2[dirIndex].size = fileSize;
        // strncpy(filePointer2[dirIndex].hash,hash,(char
        // *)sizeof(filePointer2[dirIndex].hash));
        memcpy(filePointer2[dirIndex].hash, hash,
               sizeof(filePointer2[dirIndex].hash));
        dirIndex++;
        bzero(fBuffer, 10000);
      }
    ...
    for (int i = 0; i < dirIndex; i++) {
      printf("1: %s ,2: %i ,3:", filePointer2[i].fileName,
             filePointer2[i].size);
      printHash(filePointer2[i].hash);
    }
  } else if (d == NULL) {
    ...
    if (strncmp(d_name, ".", 1) != 0 && strncmp(d_name, "..", 2) != 0) {
      ...
      if (stat(fileName, &sb) == 0 && S_ISREG(sb.st_mode)) {
        ...
        if (numManFiles == 0) {
          ...
          memcpy(filePointer[numManFiles].fileName, fileName,
                 strnlen(fileName, sizeof(fileName)));
          ...
          memcpy(temp[numManFiles].fileName, fileName,
                 strnlen(fileName, sizeof(fileName)));
          filePointer = temp;
          printf("File %i Name:%s\n", numManFiles,
                 filePointer[numManFiles].fileName);
        ...
        printf("Hash of file: %s is:", dir->d_name);
        printHash(hash);
        memcpy(filePointer[numManFiles].hash, hash,
               sizeof(filePointer[numManFiles].hash));
        printf("\n");
        bzero(fBuffer, 10000);
        numManFiles++;
      }
