...
int main(int argc, char *argv[]) {  // format is ./client website port
  ...
  char msgBuffer[1028];
  struct sockaddr_in
      serv_addr;  //, cli_addr; //struct for server and client addr
        ...
        if (msgBuffer[0] == CLIENT_MESSAGE) {
          ...
          if (remaining_len > (sizeof(file_recv) - 1)) {
            ...
            if (remaining_len >= recv.fileSize) {
              printf(
                  "Reads the File Contents here after handling the name and "
                  "size\n");
              uint32_t writeCheck;
              ...
              char *prependFileName =
                  (char *)malloc(sizeof(recv.fileName) + 28);
              snprintf(prependFileName, sizeof(recv.fileName) + 28,
                       "/home/danny/fileproj/files/%s", recv.fileName);
  ...
  if (d) {
    ...
    while ((dir = readdir(d)) != NULL) {
      if (strstr(dir->d_name, search) != NULL) {
        ...
        printHash(hash);
        fclose(fp);
        strncpy(filePointer[dirIndex].fileName, dir->d_name,
                sizeof(filePointer[dirIndex].fileName));
        filePointer[dirIndex].size = fileSize;
        // strncpy(filePointer[dirIndex].hash,hash,(char
        // *)sizeof(filePointer[dirIndex].hash));
        memcpy(filePointer[dirIndex].hash, hash,
               sizeof(filePointer[dirIndex].hash));
        dirIndex++;
        bzero(fBuffer, 10000);
      }
